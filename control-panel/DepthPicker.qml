...
//
// First the base plane, fitted to a flat plate laid on the sand: loose sand is
// never flat enough to be its own reference. Then the plate comes out and the
// bare sand is dragged over again for the extents.
//
// Selecting the region is the part that matters either way. A fit to the whole
// depth image cannot tell the sand from the rest of the room and settles on
// whatever surface dominates the view.
ColumnLayout {
    id: picker
    ...
    // "plane" drags over the plate, "extents" drags over the bare sand,
    // "corners" clicks the four individually.
    property string mode: "plane"
    ...
    Connections {
        ...
        function onStatus(key, value) {
            ...
            if (key === "depthImage") {
                ...
                picker.cornerText = "now take the plate out, grab the depth image again, " +
                                    "and drag over the bare sand"
            } else if (key === "extentsSet") {
                picker.cornerCount = 4
                picker.cornerText = "extents " + parseFloat(f[0]).toFixed(1) + " x " +
                                    parseFloat(f[1]).toFixed(1) + " cm — click a corner to re-pick"
                picker.mode = "corners"
    ...
    Label {
        ...
        font.pixelSize: 13
        opacity: 0.85
        text: picker.mode === "plane"
            ? "1. Lay a flat plate on the sand and drag a rectangle over it. The plane is fitted to that region only."
            : picker.mode === "extents"
            ? "2. Take the plate out, grab the depth image again, then drag over the bare sand to set its extents."
            : "Extents set. Click a corner to re-pick it by hand, in order: bottom left, bottom right, upper left, upper right."
    }
    ...
    Rectangle {
        ...
        Rectangle {
            id: selection
            visible: picker.mode !== "corners" && dragArea.dragging
            color: "#3399ff33"
            border.color: "#3399ff"
            border.width: 2
        }
        ...
        MouseArea {
            ...
            onReleased: function(mouse) {
                ...
                pipe.send((picker.mode === "plane" ? "fitPlane " : "setExtents ") +
