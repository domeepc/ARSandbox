	...
	try
		...
		if(illuminate)
			...
			vertexMain+="\
				/* Calculate the vertex' tangent plane equation in depth image space.\n\
				   The central difference is taken over +/-2 depth pixels rather than\n\
				   +/-1: a one-pixel difference is a high-pass filter on a depth signal\n\
				   whose residual noise is exactly what the frame filter could not\n\
				   remove, and it is the direct cause of speckle in the shading. The\n\
				   .z component is the pixel span of the difference and must track it: */\n\
				vec4 tangentDic;\n\
				tangentDic.x=texture2DRect(depthSampler,vec2(vertexDic.x-2.0,vertexDic.y)).r-texture2DRect(depthSampler,vec2(vertexDic.x+2.0,vertexDic.y)).r;\n\
				tangentDic.y=texture2DRect(depthSampler,vec2(vertexDic.x,vertexDic.y-2.0)).r-texture2DRect(depthSampler,vec2(vertexDic.x,vertexDic.y+2.0)).r;\n\
				tangentDic.z=4.0;\n\
		...
		*(ulPtr++)=glGetUniformLocationARB(result,"depthProjection");
		...
		if(drawContourLines)
			...
			*(ulPtr++)=glGetUniformLocationARB(result,"contourLineWidth");
			}
		if(drawDippingBed)
			{
			if(dippingBedFolded)
		...
		if(illuminate)
			...
			*(ulPtr++)=glGetUniformLocationARB(result,"reliefStrength");
			}
		if(waterTable!=0&&dem==0)
			{
...
SurfaceRenderer::SurfaceRenderer(const DepthImageRenderer* sDepthImageRenderer)
	:depthImageRenderer(sDepthImageRenderer),
	 drawContourLines(true),contourLineFactor(1.0f),contourLineWidth(1.6f),
	 elevationColorMap(0),
	 ...
	 illuminate(false),reliefStrength(0.35f),
...
void SurfaceRenderer::setContourLineWidth(GLfloat newContourLineWidth)
	{
	/* Set the new contour line width. This is a plain uniform and does not
	   affect shader source, so the surface settings version is not bumped: */
	contourLineWidth=newContourLineWidth;
	}

void SurfaceRenderer::setReliefStrength(GLfloat newReliefStrength)
	{
	/* Set the new relief shading strength; also a plain uniform: */
	reliefStrength=newReliefStrength;
	}
...
void SurfaceRenderer::renderSinglePass(const int viewport[4],const PTransform& projection,const OGTransform& modelview,GLContextData& contextData) const
	...
	if(drawContourLines)
		...
		/* Upload the contour line width: */
		glUniform1fARB(*(ulPtr++),contourLineWidth);
		}
	...
	if(illuminate)
		...
		/* Upload the relief shading strength: */
		glUniform1fARB(*(ulPtr++),reliefStrength);
		}
